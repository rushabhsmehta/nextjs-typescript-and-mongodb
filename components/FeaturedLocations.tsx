...
function Location(props: Partial<ILocation>) {
  ...
  return (
    <a href= {`tourpackages/${props.slug}`}  rel="noopener noreferrer">
...
export default function FeaturedLocations() {
    ...
    const filteredLocations = data.filter(location => location.tag && location.tag.includes('features'));
...
<div className='py-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 rounded-md' style={{ overflowX: 'auto' }}>
    ...
    {filteredLocations.map(location => (
        <Location {...location} key={`locations-${location._id}`} />
      ))}
  </div>
    )
}
